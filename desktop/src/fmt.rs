...
//!
//! Every one of them depends on a global switch — the Dec/Hex button, the "hex
//! bytes" option, the "number blocks from 0" option — so they take the flags
//! explicitly and the call sites read them off the store. That is the rule the
//! migration plan asks for: the options live in the state struct the row
//! builders read, not in the call sites.
...
/// `fmtByte`: a flag or checksum byte. Hex when either switch says so, else a
/// zero-padded decimal, the way the web editor shows them.
...
mod tests {
    ...
    fn numbers_follow_the_switch() {
        ...
        assert_eq!(byte(0xff, true, false), "FF");
    }
